...
function wsproxy(mid,settings) {
    ...
    if (mid != null) { 
        ...
        log.trace("[WSProxy] - Init WSProxy on BusinessUnit "+ mid);
    } else {
        log.trace("[WSProxy] - Init WSProxy");
    }
    ...
    this.retrievableCols = function(objectType) {
        ...
        // do we have the cols already in cache?
        if (Array.isArray(this.cachedCols[objectType]) && this.cachedCols[objectType].length > 0) {
            log.trace("[retrievableCols] - Using cached cols for objectType [" + objectType + "]");
            return this.cachedCols[objectType];
        }

        // check if we have deCols
        if (de_wsproxyCols) {
            var dataRows = Platform.Function.LookupRows(de_wsproxyCols,'ObjectType',objectType),
                cols = dataRows[0]["Cols"],
                lastUpdate = dataRows[0]["Last Update"];

            // return DataExtension value but only if it is less than 7 days old
            if (cols.length > 0 && dateDiffInDays(new Date(lastUpdate), dateNow) < 7) {
                cols = cols.split(",");
                this.cachedCols[objectType] = cols;
                log.trace("[retrievableCols] - Using DataExtension cols for objectType [" + objectType + "]");
                return cols;
            }

        } else {
            log.warn("[retrievableCols] - For better usage and performance, please add a DataExtension for cols storage and add it to the setting de.wsproxyCols with fields: [(PK) ObjectType: Text 50] [Cols: Text 4000] [Last Update: Date]");
        }
        
        // get fresh set of cols
        var cols = [],
            req = this.api.describe(objectType),
        ...
        if (objectType == 'Automation') {
            log.trace('[retrievableCols] - Using retrievableCols on Automation results in cols which are not retrievable. ObjectID must be ProgramID while Client and Schedule are also invalid. Adding legacy ProgramId to retrieve ObjectId'); 
            cols.push('ProgramID','LastRunTime');
        }

        // describe of AutomationInstance is bugged
        if (objectType == 'AutomationInstance') {
            log.trace('[retrievableCols] - Using retrievableCols on AutomationInstance results in bugged cols. Set hardcoded cols'); 
            cols.push("Name","Status","CompletedTime","CustomerKey","StatusMessage","StartTime","ScheduledTime","StatusLastUpdate");
        }

        // update DataExtension if DataExtension exist
        if (de_wsproxyCols) {
            log.trace('[retrievableCols] - Renew DataExtension data for objectType [' + objectType + ']');
            Platform.Function.UpsertData(
                de_wsproxyCols,
                ["ObjectType"], [objectType],
                ["Cols", "Last Update"], [cols.join(","), dateNow]
            );
        }
     ...
     * ATTENTION:
     *
     * WSPROXY does not work well with assets in Automation Studio.
     * If an asset cannot be found, the REST API will be utilised.
     * 
    ...
    this.retrieveAsset = function(id) {
        ...
        for (var i = 0; i < property.length; i++) {
            ...
            if( req.Status == "OK" && req.Results.length > 0 ) {
                log.trace("[retrieveAsset] - Successfully retrieved Asset [" +id+ "]", req);
                return req;
            }
        }
    ...
    this.createTriggeredSendDefinition = function(name,tdsEmailId,description,tdsDataExtensionId,sendClassificationId,folderId) {
        // using rest if needed and load it to cache
        var rest = new sfmcapi(),
            categoryID = (Number.isInteger(folderId) && this.isTriggeredSendFolder(folderId)) ? folderId : this.settings.folderId['Triggered Sends'];

        // check if tds already exists
        if (this.isTriggeredSendDefinition(name)) {
            log.debug('(createTriggeredSendDefinition)\n\tINFO: createTriggeredSendDefinition '+name+' already exists');
            ...
            // get asset id via rest
            var assetKey = rest.retrieveAssetCustomerKey(tdsEmailId);
     ...
     * @returns {boolean}
     */
    this.deleteTriggeredSendDefinition = function(id) {
        var customerKey = this.retrieveTriggeredSendDefinitionCustomerKey(id);

        // only delete if it exists
        if (customerKey) {
            // need to change the status to Inactive if not already in order to delete
            this.updateTriggeredSendDefinitionStatus(customerKey, "Inactive");

            // delete TDS
            var resp =  this.api.deleteItem("TriggeredSendDefinition", { "CustomerKey": customerKey });
            
            if (resp.Status == 'OK') {
                log.trace("[deleteTriggeredSendDefinition] - TriggeredSend [" + id+ "] has been deleted successfully");
                return true;
            } else {
                log.warn("[deleteTriggeredSendDefinition] - TriggeredSend can not be deleted:", resp.Results[0].StatusMessage);
                return false;
            }
        } else {
            log.trace("[deleteTriggeredSendDefinition] - TriggeredSend [" + id+ "] was not found. Nothing to delete");
            return true
        }
    }
    ...
    this.updateTriggeredSendDefinitionStatus = function(id, status) {
        ...
        if (customerKey) {
            ...
            try {
                ...
                if( req.Status == 'OK' ) { 
                    log.trace("[updateTriggeredSendDefinitionStatus] - Status for TDS [" +id+ "] has been updated to [" +status+ "]");
                    return true;

                } else {
                    log.info("[updateTriggeredSendDefinitionStatus] - Failed to update status to [" +status+ "] for TDS [" +id+ "]", req.Results[0].StatusMessage);
                    return false;
                }
            } catch(err) {
                log.info("[updateTriggeredSendDefinitionStatus] - Failed to update status to [" +status+ "] for TDS [" +id+ "]", err);
                return false;
            }
        }
        return false;
    ...
    this.retrieveTriggeredSendDefinitionProperty = function(id,property) {
        ...
        if( req.Status == "OK" && req.Results.length == 1 && prop ) {
            log.trace("[retrieveTriggeredSendDefinitionProperty] - Successfully retrieved "+property+" for TriggeredSendDefinition [" +id+ "] with value [" +prop+ "]" );
            return prop;
        } else {
            log.trace("[retrieveTriggeredSendDefinitionProperty] - Unable to find "+property+" for TriggeredSendDefinition [" +id+ "]", req );
            return null;
        }
    };
